...
/**
  ...
  * @retval 无
  */
//void ImagDataSentToPhone(uint16_t width,uint16_t height)//width：320 height：240 传过来的就是320 240
//{
//	//uint8_t data[][] ={0} ;
//	uint16_t i, j; 
...
////			printf("%X",Camera_Data);
//			datal[0] = Camera_Data & 0xff;
//			datah[0] = Camera_Data>>8 & 0xff;  
//			M8266WIFI_SPI_Send_Data(datah,1,1,NULL);//发送像素点的高位
//			M8266WIFI_SPI_Send_Data(datal,1,1,NULL);//发送像素点的低位			
//		}
//	}
//}
void ImagDataSentToPhone(uint16_t width,uint16_t height)//width：320 height：240 传过来的就是320 240
{
	 unsigned char data[640] ={0};
...
//			printf("%X",Camera_Data);
		...
		M8266WIFI_SPI_Send_Data(data,640,1,NULL);
	}
